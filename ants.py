...
class Ant(Insect):
    ...
    def __init__(self, health=1, double_damage = False):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.is_container and not self.is_container and ContainerAnt.can_contain(place.ant, self):
                ContainerAnt.store_ant(place.ant, self)
                self.place = place
                return

            elif not place.ant.is_container and self.is_container and ContainerAnt.can_contain(self, place.ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if self.lower_bound:
            i = 0
            while i < self.lower_bound and not curr_place.is_hive:
                curr_place = curr_place.entrance
                i+=1
        if self.upper_bound:
            index = self.lower_bound if self.lower_bound else 0
            while not curr_place.is_hive and index <= self.upper_bound:
                if curr_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            ...
            #Deal amount + damage   #.reduce_health
        else:
            # deal amount 
            for bee in self.place.bees[:]:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while current.exit:
            ...
            if current.ant:
                Ant.double(current.ant)
                current.ant.double_damage = True
        # END Problem 12
